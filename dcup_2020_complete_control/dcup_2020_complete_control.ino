...
int led2 = 0;
  ...
  int32_t PID_position_p[3] = {300, 500, 950};
...
ros::Subscriber<sensor_msgs::Joy> joy_sub("/bluetooth_teleop/joy",  &joy_cb);
ros::Subscriber<damm_msgs::Light> light_sub("damm_light",  &light_cb);
ros::Publisher  position_pub("damm_position",  &srvo_pos_msg);

unsigned long joy_timer = 0;
unsigned long goal_timer = 0;
...
void setup() 
{
  ...
  // Send settting values to dynamixels
  dxl_wb.syncWrite(write_handler_index, dxl_id, SRV_COUNT, PID_position_p, 1, &log);
  dxl_wb.syncWrite(integral_handler_index, dxl_id, SRV_COUNT, PID_position_i, 1, &log);
  dxl_wb.syncWrite(return_handler_index, dxl_id, SRV_COUNT, return_delay, 1, &log);
  //Set Home positions
...
void joy_cb( const sensor_msgs::Joy& joy_msg){
    if ((joy_timer + 10) > millis()) return;
    joy_timer = millis();
    float dxl1 = joy_msg.axes[2];  //right thumb
    float right = joy_msg.axes[4] - 1 ; //right trigger
    float left = joy_msg.axes[3] - 1; //left trigger
    float dxl2  = left - right;
    float lin_raw = joy_msg.axes[5];
    int lin_srvo = 0;
    if (lin_raw > 0.2 ||  lin_raw < -0.2) {
...
void goal_cb( const damm_msgs::Servos& goal_msg){
    ...
    goal_position[0] = fmax(fmin(dxl1, DXL1_MAX),DXL1_MIN);
    goal_position[1] = fmax(fmin(dxl2 , DXL2_MAX),DXL2_MIN);
    lin_srvo_goal = fmax(fmin(lin_raw, LIN_MAX), LIN_MIN);
    if (dxl3 == 1){
      goal_position[2] = CUT_LEFT;
    }else if (dxl3 == -1){
      goal_position[2] = CUT_RIGHT;
